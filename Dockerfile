...
RUN apk add --no-cache git python3 ca-certificates libffi-dev openssl-dev && \
    ...
    pip3 install --no-cache-dir homeassistant==0.76.2 && \
    pip3 install --no-cache-dir home-assistant-frontend==20180820.0 sqlalchemy==1.2.10 distro==1.3.0 aiohttp_cors==0.7.0 jsonrpc-async==0.6 samsungctl[websocket]==0.7.1 pychromecast==2.1.0 paho-mqtt==1.3.1 rxv==0.5.1 jsonrpc-websocket==0.6 wakeonlan==1.0.0 websocket-client==0.37.0 && \
